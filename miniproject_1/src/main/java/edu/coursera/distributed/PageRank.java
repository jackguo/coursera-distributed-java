...
public final class PageRank {
    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        JavaPairRDD<Integer, Double> intermediate = sites.join(ranks).flatMapToPair(p -> {
            List<Tuple2<Integer, Double>> res = new ArrayList<>();
            int numOfEdges = p._2._1.getNEdges();
            p._2._1.edgeIterator().forEachRemaining(siteId -> res.add(new Tuple2<Integer, Double>(siteId, p._2._2 / numOfEdges)));
            return res;
        });
        return intermediate.reduceByKey(Double::sum).mapValues(r -> 0.15 + 0.85 * r);
    }
}
